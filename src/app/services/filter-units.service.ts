...
export class FilterUnitsService {
  ...
  filtersUnits(unit: Location, open_hour: string, close_hour: string) {
  ...
  filter(results: Location[], showClosed: boolean, hour: string){
    ...
    if(hour){
      ...
      return intermediateResults.filter(location => this.filtersUnits(location, OPEN_HOUR, CLOSE_HOUR));
    } else {
